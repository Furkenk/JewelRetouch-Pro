...
const App: React.FC = () => {
  ...
  const [selectedJewelryAsset, setSelectedJewelryAsset] = useState<number | null>(null);

  const jewelryAssets = [
    { id: 1, name: "Model 1", preview: "/1.png", promptSuffix: "- LUXURY LIFESTYLE RING PORTRAIT — REAL SKIN, REAL SCALE, ZERO STYLIZATION\n\nRING INTEGRITY — NON-NEGOTIABLE\nThe ring must remain 100% identical to the uploaded reference:\n• exact geometry\n• exact prongs\n• exact stone layout\n• exact metal tone\n• no recoloring\n• no added reflections\n• no stylization of any kind\n\nSTRICT MICRO-SCALE LOCK\nThe ring must render at true-to-life elegant proportions:\n• Overall scale = petite, delicate, dainty (NOT bold)\n• Band + halo width = slender, refined\nPrevent ANY bulky or oversized appearance.\n\nRing must sit naturally with:\n• correct skin compression\n• perfect contact shadow\n• no floating\n• no warping\n\nSKIN QUALITY — NATURAL & YOUTHFUL (NO WRINKLES)\nHands must show:\n• soft, youthful skin (25–35 age range)\n• perfect natural texture (pores visible, but no visible wrinkles, veins, or blemishes)\n• subtle highlights on the skin surface\n• elegant hand posture, professional jewelry modeling pose\n\nLIGHTING & DEPTH\n• Cinematic luxury studio lighting\n• Soft, diffused highlights\n• Shallow depth of field (slight bokeh on background)\n• Neutral, high-end environment" },
    { id: 2, name: "Model 2", preview: "/2.png", promptSuffix: "REFINED DAINTY ASSEMBLY. Reduce the entire ring assembly to 70% of reference scale. Delicate, minimalist accent." },
    { id: 3, name: "Model 3", preview: "/3.png", promptSuffix: "CLASSIC PROPORTIONS. Natural fit, soft skin lighting, elegant hand pose." },
    { id: 4, name: "Model 4", preview: "/4.png", promptSuffix: "MODERN ELITE. Sharp focus on geometry, high-end luxury lifestyle background." },
    { id: 5, name: "Model 5", preview: "/5.png", promptSuffix: "REGAL BOLD ASSEMBLY. Proportional lock 1:1, voluminous presence, maximum luxury heritage." }
  ];
  ...
  const handleRetouch = async () => {
    ...
    try {
      if (state.activeMode === 'retoucher') {
        ...
        currentCounter++;
        let assetPrompt = undefined;
        if (state.fashionReferences.length > 0) {
          if (selectedJewelryAsset !== null && typeof selectedJewelryAsset === 'number' && selectedJewelryAsset < state.fashionReferences.length) {
            assetPrompt = `PRIMARY PRODUCT FOCUS: Use the product details from the selected reference (image index ${selectedJewelryAsset + 1}). Maintain its exact proportions, stone cut, and setting style.`;
          }
        } else if (selectedJewelryAsset) {
          assetPrompt = jewelryAssets.find(a => a.id === selectedJewelryAsset)?.promptSuffix;
        }
    ...
    <div className={`min-h-screen flex flex-col transition-colors duration-500 ${state.isDarkMode ? 'dark bg-slate-950 text-white' : 'bg-[#fcfcfc] text-gray-900'}`}>
        ...
        {state.retouchedResults.length > 0 ? (
          <div className="space-y-12 animate-in fade-in slide-in-from-bottom-4 duration-700">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
              <div className="space-y-2">
                <h2 className={`text-3xl font-extrabold tracking-tight ${state.isDarkMode ? 'text-white' : 'text-gray-900'}`}>{t.gallery.title}</h2>
                <div className="flex items-center gap-4">
                  <p className={`text-[10px] font-bold uppercase tracking-widest ${state.isDarkMode ? 'text-slate-500' : 'text-gray-400'}`}>{state.retouchedResults.length} {t.gallery.rendered}</p>
              ...
              <div className="lg:col-span-3 grid grid-cols-1 md:grid-cols-2 gap-10">
                    ...
                    <div className="flex justify-between items-end">
                      ...
                      <div className="flex items-center gap-4">
                        ...
                        >
                          ...
                          {t.gallery.previewLink}
                        </button>
                        <button onClick={() => downloadOne(res.url, res.sourceName)} className={`text-[10px] font-black uppercase tracking-widest hover:underline ${state.isDarkMode ? 'text-blue-400' : 'text-blue-600'}`}>{t.gallery.download}</button>
                      </div>
                    </div>
                ...
                {state.isProcessing && (
                  <div className={`aspect-square border-2 border-dashed rounded-[2.5rem] flex flex-col items-center justify-center space-y-4 ${
                    ...
                    <p className={`text-[10px] font-black uppercase tracking-[0.2em] ${state.isDarkMode ? 'text-slate-500' : 'text-gray-400'}`}>{t.common.next}</p>
                  </div>
                )}
              </div>
              ...
              <div className="lg:col-span-1 space-y-6">
                {state.activeMode !== 'upscale' && (
                  <div className={`p-6 rounded-[2rem] border shadow-xl space-y-6 sticky top-32 transition-colors duration-500 ${
                    ...
                    <h3 className={`text-sm font-black uppercase tracking-widest ${state.isDarkMode ? 'text-white' : 'text-gray-900'}`}>{t.adjustments.title}</h3>
                    ...
                    <div className="space-y-4">
                      <div className={`p-4 rounded-2xl flex flex-col gap-4 border ${state.isDarkMode ? 'bg-slate-800 border-slate-700' : 'bg-gray-50 border-gray-100'}`}>
                        <span className={`text-[10px] font-black uppercase tracking-widest text-center ${state.isDarkMode ? 'text-white' : 'text-gray-900'}`}>{t.sidebar.productScale}</span>
                      ...
                      >
                        {state.isProcessing ? t.sidebar.processing : t.sidebar.tryGenerate}
                      </button>
                    </div>
                  </div>
                )}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
                              ...
                              <div className="space-y-2">
                                ...
                                <div className={`relative border-2 border-dashed rounded-xl py-3 px-4 text-center cursor-pointer overflow-hidden ${state.isDarkMode ? 'bg-slate-900 border-slate-700' : 'bg-white border-gray-100'}`}>
                                  ...
                                  {state.watermark.image?.preview ? (
                                    ...
                                    <span className="text-[8px] font-black uppercase text-gray-400">{t.common.uploadLogo}</span>
                                  )}
                                </div>
                              </div>
                            )}
                  ...
                  <>
                    ...
                    <div className="space-y-6">
                      <div>
                        ...
                        {fashionOptions.productTypes.includes('ring') && (
                          <div>
                            <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-3 block">{t.fashion.fingerSize}</label>
                            <div className="grid grid-cols-3 gap-2">
                              {(['normal', 'medium', 'large'] as const).map((size) => (
                                <button
                                  key={size}
                                  onClick={() => setFashionOptions({ ...fashionOptions, fingerSize: size })}
                                  className={`py-2 px-3 rounded-xl text-[10px] font-black uppercase tracking-tighter transition-all border ${
                                    fashionOptions.fingerSize === size ? 'bg-black text-white border-black' : 'bg-white text-gray-300 border-gray-100'
                                  }`}
                                >
                                  {t.fashion.fingerSizes[size]}
                                </button>
                              ))}
                            </div>
                          </div>
            ...
            <div className="lg:col-span-8 space-y-8 order-1 lg:order-2">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                {state.activeMode === 'retoucher' ? (
                  <>
                    <div className="space-y-2">
                      <div className="flex justify-between items-end mb-2">
                          <h3 className={`text-sm font-black uppercase tracking-widest ${state.isDarkMode ? 'text-white' : 'text-gray-900'}`}>{t.common.productInput} {options.isBulkMode && '(Bulk)'}</h3>
                          {state.rawImages.length > 0 && <button onClick={() => setState(p => ({ ...p, rawImages: [] }))} className="text-[10px] text-red-500 font-bold uppercase transition-colors hover:text-red-400">{t.common.clearAll}</button>}
                      </div>
                        ...
                        state.isDarkMode 
                          ...
                          {state.rawImages.length > 0 ? (
                            ...
                            <div className="flex-1 flex flex-col items-center justify-center text-center p-8 space-y-4">
                              ...
                              <p className="text-[11px] font-black text-gray-400 uppercase tracking-widest">{options.isBulkMode ? t.common.dropMultiple : t.common.clickUploadMaster}</p>
                            </div>
                          )}
                      </div>
                    </div>
                    ...
                    <div className="space-y-2">
                      <div className="flex justify-between items-end mb-2">
                          <h3 className={`text-sm font-black uppercase tracking-widest ${state.isDarkMode ? 'text-white' : 'text-gray-900'}`}>{t.common.styleRef}</h3>
                          {state.referenceImage.base64 && <button onClick={() => setState(prev => ({ ...prev, referenceImage: { file: null, preview: null, base64: null } }))} className="text-[10px] text-red-500 font-bold uppercase transition-colors hover:text-red-400">{t.common.clear}</button>}
                      </div>
                      <div className={`relative border-2 border-dashed rounded-[2.5rem] group transition-all overflow-hidden min-h-[300px] flex flex-col p-4 ${
                        state.isDarkMode 
                          ? 'bg-slate-900 border-slate-800 hover:border-white' 
                          ...
                          {state.referenceImage.base64 ? (
                            <div className={`flex-1 rounded-2xl overflow-hidden border relative ${state.isDarkMode ? 'bg-slate-800 border-slate-700' : 'bg-gray-50 border-gray-100'}`}>
                              ...
                              <div className="absolute inset-0 bg-black/5 flex items-end p-4">
                                  <span className={`text-[10px] px-2 py-1 rounded font-black truncate ${state.isDarkMode ? 'bg-slate-900/90 text-slate-400' : 'bg-white/90 text-gray-500'}`}>{state.referenceImage.name || t.common.styleRef}</span>
                              </div>
                            </div>
                          ) : (
                            <div className="flex-1 flex flex-col items-center justify-center text-center p-8 space-y-4">
                              ...
                              <p className="text-[11px] font-black text-gray-400 uppercase tracking-widest">{t.common.clickUploadRef}</p>
                              <p className="text-[9px] font-bold text-gray-400 uppercase tracking-widest">{t.sidebar.lighting.balanced}</p>
                            </div>
                          )}
                      </div>
                    </div>
                  ...
                  <div className="md:col-span-2 space-y-8">
                    <div className="space-y-2">
                        <div className="flex justify-between items-end mb-2">
                            <h3 className={`text-sm font-black uppercase tracking-widest ${state.isDarkMode ? 'text-white' : 'text-gray-900'}`}>{t.fashion.jewelryAssets}</h3>
                            {state.fashionReferences.length > 0 && <button onClick={() => setState(p => ({ ...p, fashionReferences: [] }))} className="text-[10px] text-red-500 font-bold uppercase transition-colors hover:text-red-400">{t.common.clearAll}</button>}
                        </div>
                            ...
                            {state.fashionReferences.length > 0 ? (
                              <div className="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-6 gap-4">
                                ...
                                <div className="aspect-square border-2 border-dashed border-gray-100 rounded-2xl flex flex-col items-center justify-center text-gray-300 bg-gray-50/50">
                                    ...
                                    <span className="text-[8px] font-black uppercase">{t.common.addMore}</span>
                                </div>
                              </div>
                            ) : (
                              <div className="flex-1 flex flex-col items-center justify-center text-center p-8 space-y-4">
                                ...
                                <div className="space-y-1">
                                    <p className={`text-[10px] font-black uppercase tracking-widest ${state.isDarkMode ? 'text-slate-500' : 'text-gray-900'}`}>{t.common.uploadJewelry}</p>
                                </div>
                              </div>
                            )}
                        </div>
                    ...
                    {fashionOptions.modelStyle === 'style-reference' && (
                      <div className="animate-in slide-in-from-top-4 duration-500">
                        <ImageUploader 
                          label={t.common.modelPoseRef} 
                          description={t.common.poseDesc}
                    ...
                    {fashionOptions.outfitType === 'reference' && (
                      <div className="animate-in slide-in-from-top-4 duration-500">
                        <ImageUploader 
                          label={t.common.outfitRef} 
                          description={t.common.outfitDesc}
                    ...
                    {fashionOptions.hasAdditionalAccessory && (
                      <div className="animate-in slide-in-from-top-4 duration-500">
                        <ImageUploader 
                          label={t.common.accessoryRef} 
                          description={t.common.accessoryDesc}
                    ...
                    {fashionOptions.faceType === 'reference' && (
                      <div className="animate-in slide-in-from-top-4 duration-500">
                        <div className="flex justify-between items-center mb-4">
                          <div>
                            <h3 className={`text-sm font-black ${state.isDarkMode ? 'text-white' : 'text-gray-900'}`}>{t.common.faceRef}</h3>
                            <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">{t.common.faceDesc}</p>
                          </div>
                          <button 
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-4">
                          {state.faceReferences.map((ref, idx) => (
                            <div key={idx} className={`relative aspect-square rounded-2xl overflow-hidden border group ${state.isDarkMode ? 'border-slate-800' : 'border-gray-100'}`}>
                          ...
                          {state.faceReferences.length < 10 && (
                            <label className={`aspect-square rounded-2xl border-2 border-dashed flex flex-col items-center justify-center cursor-pointer transition-colors ${
                              state.isDarkMode ? 'border-slate-800 hover:border-slate-700 hover:bg-slate-800/50' : 'border-gray-200 hover:border-gray-300 hover:bg-gray-50'
                            }`}>
                              <input 
                                type="file" 
                                className="hidden" 
                              ...
                              <span className="text-[10px] font-black text-gray-400 uppercase tracking-widest">{t.common.addFace}</span>
                            </label>
                          )}
                        </div>
                      </div>
                  ...
                  <div className="md:col-span-2 space-y-8">
                    <div className="space-y-2">
                      <div className="flex justify-between items-end mb-2">
                        <h3 className={`text-sm font-black uppercase tracking-widest ${state.isDarkMode ? 'text-white' : 'text-gray-900'}`}>{t.common.imageToUpscale}</h3>
                        {state.upscaleImage.base64 && <button onClick={() => setState(p => ({ ...p, upscaleImage: { file: null, preview: null, base64: null } }))} className="text-[10px] text-red-500 font-bold uppercase hover:text-red-400">{t.common.clear}</button>}
                      </div>
                      <div className={`relative border-2 border-dashed rounded-[2.5rem] group transition-all overflow-hidden min-h-[300px] flex flex-col p-4 ${
                        ...
                        {state.upscaleImage.base64 ? (
                          ...
                          <div className="flex-1 flex flex-col items-center justify-center text-center p-8 space-y-4">
                            ...
                            <p className={`text-[11px] font-black uppercase tracking-widest ${state.isDarkMode ? 'text-slate-600' : 'text-gray-400'}`}>{t.common.uploadForClarity}</p>
                          </div>
                        )}
                      </div>
                    </div>
              ...
              {state.activeMode === 'fashion' && fashionOptions.productTypes.includes('ring') && (
                <div className={`lg:col-span-12 p-8 border rounded-[3rem] transition-colors duration-500 shadow-sm ${
                  state.isDarkMode 
                    ? 'border-slate-800 bg-slate-900/20 text-white' 
                    : 'border-gray-50 bg-white/30 text-gray-900'
                }`}>
                   <div className="flex justify-between items-center mb-6 px-4">
                     <h3 className="text-xs font-black uppercase tracking-widest">{t.fashion.selectModel}</h3>
                     <p className="text-[8px] font-black uppercase tracking-[0.2em] opacity-30">Perspective Locking Engine v5.2 Active</p>
                   </div>
                   
                   <div className="flex flex-wrap gap-4 px-4">
                      {state.fashionReferences.length > 0 ? (
                        state.fashionReferences.map((asset, idx) => (
                          <button
                            key={idx}
                            onClick={() => setSelectedJewelryAsset(idx)}
                            className={`w-20 h-20 rounded-2xl overflow-hidden border-2 transition-all p-1 relative group ${
                              selectedJewelryAsset === idx 
                                ? 'border-indigo-500 scale-105 shadow-xl ring-4 ring-indigo-500/10' 
                                : 'border-transparent opacity-60 hover:opacity-100 hover:scale-105'
                            }`}
                          >
                            <img src={asset.preview!} className="w-full h-full object-cover rounded-xl" />
                            <div className={`absolute inset-0 flex items-center justify-center transition-opacity ${selectedJewelryAsset === idx ? 'opacity-100' : 'opacity-0'}`}>
                        ...
                        <div className="flex items-center gap-4 py-4">
                           {jewelryAssets.map((asset) => (
                            <button
                              key={asset.id}
                              onClick={() => setSelectedJewelryAsset(asset.id)}
                              className={`w-16 h-16 rounded-xl overflow-hidden border-2 transition-all p-0.5 ${
                                selectedJewelryAsset === asset.id ? 'border-black' : 'border-transparent opacity-50 hover:opacity-100'
                              }`}
                            >
                              <img src={asset.preview} className="w-full h-full object-cover rounded-lg" />
                            </button>
                          ))}
                        </div>
                      )}
              ...
              >
                {activeMenu === 'main' ? (
                    ...
                    >
                      ...
                      <div className="flex items-center space-x-2">
                        <span className="text-[8px] font-bold uppercase opacity-50">
                          {state.language === 'tr' ? 'Turkish' : state.language === 'en' ? 'English' : state.language === 'fr' ? 'Français' : state.language === 'it' ? 'Italiano' : state.language === 'es' ? 'Español' : state.language === 'de' ? 'Deutsch' : state.language === 'pt' ? 'Português' : state.language}
                        </span>
                        <svg className="w-4 h-4 opacity-50" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" /></svg>
                      </div>
                    </button>
                    <button onClick={() => setShowPricing(true)} className={`w-full flex items-center justify-between px-4 py-3 rounded-xl transition-colors ${state.isDarkMode ? 'hover:bg-slate-700 text-white' : 'hover:bg-gray-50 text-gray-700'}`}>
                      <span className="text-[10px] font-black uppercase tracking-widest">{t.profile.pricing}</span>
                      <svg className="w-4 h-4 opacity-50" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" /></svg>
                    </button>
                  ...
                  <div className="p-2 space-y-1">
                    ...
                    <button 
                      onClick={() => { setState(s => ({ ...s, language: 'pt' })); setActiveMenu('main'); setShowProfileMenu(false); }}
                      className={`w-full flex items-center justify-between px-4 py-3 rounded-xl transition-colors ${state.language === 'pt' ? (state.isDarkMode ? 'bg-slate-700' : 'bg-gray-100') : ''} ${state.isDarkMode ? 'hover:bg-slate-700 text-white' : 'hover:bg-gray-50 text-gray-700'}`}
                    >
                      <span className="text-[10px] font-black uppercase tracking-widest">Português</span>
                      {state.language === 'pt' && <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" /></svg>}
                    </button>
                  </div>
                )}
              </motion.div>
      ...
      <div className="fixed bottom-8 right-8 z-50 flex items-center bg-white dark:bg-slate-800 p-1.5 rounded-2xl shadow-2xl border border-gray-100 dark:border-slate-700">
        ...
        >
          ...
          <span className="text-[10px] font-black uppercase tracking-widest hidden md:block">{t.common.light}</span>
        </button>
        <button 
          onClick={() => !state.isDarkMode && toggleDarkMode()}
          className={`flex items-center space-x-2 px-3 py-1.5 rounded-xl transition-all ${state.isDarkMode ? 'bg-white text-slate-900 shadow-lg' : 'text-gray-400 hover:text-gray-900'}`}
        >
          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M20.354 15.354A9 9 0 018.646 3.646 9.003 9.003 0 0012 21a9.003 9.003 0 008.354-5.646z" /></svg>
          <span className="text-[10px] font-black uppercase tracking-widest hidden md:block">{t.common.dark}</span>
        </button>
      </div>
