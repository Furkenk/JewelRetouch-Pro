                ...
                {(state.activeMode === 'fashion' || state.activeMode === 'prompt-to-image') && (
                  <div className="pt-4 border-t border-gray-100 space-y-4">
                    <div className="flex items-center justify-between p-4 bg-gray-50 rounded-2xl border border-gray-100">
                      <div>
                        <p className="text-[10px] font-black text-gray-900 uppercase tracking-widest">Model Consistency</p>
                        <p className="text-[8px] text-gray-400 font-bold uppercase mt-1">LOCK IDENTITY & CHARACTER</p>
                      </div>
                      <button 
                        onClick={() => setState(s => ({ ...s, isConsistentModelEnabled: !s.isConsistentModelEnabled }))}
                        className={`w-10 h-5 rounded-full transition-all relative ${state.isConsistentModelEnabled ? 'bg-black' : 'bg-gray-200'}`}
                      >
                        <div className={`absolute top-1 w-3 h-3 bg-white rounded-full transition-all ${state.isConsistentModelEnabled ? 'left-6' : 'left-1'}`} />
                      </button>
                    </div>
