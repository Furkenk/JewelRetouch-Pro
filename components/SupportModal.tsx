...
const SupportModal: React.FC<SupportModalProps> = ({ isOpen, onClose, language, isDarkMode }) => {
  ...
  const [chat, setChat] = useState<{sender: 'ai' | 'user', text: string}[]>([
    { sender: 'ai', text: t.support.welcome }
  ]);
  ...
  const handleSendLive = () => {
    ...
    setTimeout(() => {
      ...
      const isSiteRelated = lowerMsg.includes('fiyat') || lowerMsg.includes('price') || 
                          lowerMsg.includes('plan') || lowerMsg.includes('nasıl') || 
                          lowerMsg.includes('how') || lowerMsg.includes('moda') || 
                          lowerMsg.includes('fashion') || lowerMsg.includes('retouch') ||
                          lowerMsg.includes('yüzük') || lowerMsg.includes('ring');

      if (isSiteRelated) {
        setChat(prev => [...prev, { 
          sender: 'ai', 
          text: t.support.siteExpertise
        }]);
      } else {
        setChat(prev => [...prev, { 
          sender: 'ai', 
          text: t.support.siteExpertiseFail
        }]);
      }
    }, 1500);
  };
